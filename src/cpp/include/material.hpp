...
/**
 * @enum The Direction of provenance of the Ray (Photon or actual ray for ray
 * tracing)
 */
...
class BxDF
{
      ...
      public:
        ...
        static float absCosTheta(const Vec3f& v)
        {
        ...
        static bool refract(const Vec3f& v,
                            const Vec3f& n,
                            float iorI,
                            float iorT,
                            Vec3f& t)
        {
                ...
                const Vec3f t_p =
                  -std::sqrt(std::max(1.0f - length2th, 0.0f)) * n;
                t = t_h + t_p;
                return true;
        }
        ...
        {
                const float f0 = (iorI - iorT) * (iorI - iorT) /
                                 ((iorI + iorT) * (iorI + iorT));
                return f0 + (1.0f - f0) *
                              pow5(std::max(1.0f - std::abs(cosThetaI), 0.0f));
        }
        ...
        // evaluate BxDF
        virtual Vec3f evaluate(Vec3f& wo,
                               Vec3f& wi,
        ...
        // NOTE: used for drawing fresnel reflection nicely at low number of
        // samples
...
class Lambert : public BxDF
{
      ...
      public:
        explicit Lambert(const Vec3f& rho)
          : BxDF(BxDFType::DIFFUSE)
          , rho(rho)
        {
        }

        Vec3f evaluate(Vec3f& wo,
                       Vec3f& wi,
...
class Phong : public BxDF
{
      ...
      public:
        explicit Phong(const Vec3f& rho,
...
class Mirror : public BxDF
{
      ...
      public:
        explicit Mirror(const Vec3f& rho)
          : BxDF(BxDFType::SPECULAR)
          , rho(rho)
        {
        }
      ...
      public:
          ...
          TransportDirection& transport_dir) const override
        {
                ...
                if (refract(wo, n, iorO, iorI, tr)) {
                        ...
                        ret.emplace_back(
                          tr, (1.0f - fr) * scaling * rho / absCosTheta(tr));
                } else {
      ...
      public:
                              ...
                              float& pdf) const override
        {
                ...
                if (sampler.getNext1D() < reflectance) {
                        ...
                        pdf = 1.0f;
                        return rho / absCosTheta(wi) +
                               randomInterval(-roughness, roughness);
                }
                // refraction
                else {
                        Vec3f tr;
