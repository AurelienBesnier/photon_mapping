...
template<typename T>
inline Vec2<T>
operator+(const Vec2<T>& v1, float k)
{
...
template<typename T>
inline Vec2<T>
operator+(float k, const Vec2<T>& v2)
{
        return v2 + k;
}
...
template<typename T>
inline Vec3<T>
operator+(const Vec3<T>& v1, float k)
{
...
template<typename T>
inline Vec3<T>
operator+(float k, const Vec3<T>& v2)
{
        return v2 + k;
}
...
inline void
orthonormalBasis(Vec3f& n, Vec3f& t, Vec3f& b)
{
        if (std::abs(n[1]) < 0.9f) {
...
inline Vec3f
worldToLocal(const Vec3f& v, const Vec3f& lx, const Vec3f& ly, const Vec3f& lz)
{
...
inline Vec3f
localToWorld(Vec3f v, Vec3f lx, Vec3f ly, Vec3f lz)
{
        Vec3f ret;
        for (int i = 0; i < 3; ++i) {
...
inline Vec3f
sphericalToCartesian(float theta, float phi)
{
...
inline float
randomInterval(float min, float max)
{
        if (min == max) {
                return max;
        }
