...
class Camera
{
      public:
         ...
         * @brief Samples a point in the image to render from the camera view
         * point.
                       ...
                       const Scene& scene) const
        {
                ...
                ray = Ray(origin + offset,
                          lower_left_corner + uv[1] * horizontal +
                            uv[0] * vertical - origin - offset);
                IntersectInfo info;
                return scene.intersect(
                  ray, info); // Check if the ray intersects with the scene
        }
      ...
      private:
        ...
        Vec3f lower_left_corner; ///< the lower left corner of the image from
