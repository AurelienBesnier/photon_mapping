...
// SplitMix
inline uint64_t
nextSplitMix(pcg32_random_t* rng)
{
...
inline uint64_t
nextXoroshiro(uint64_t* seed1, uint64_t* seed2)
{
        uint64_t s1 = seed1[0];
        uint64_t s0 = seed2[0];
...
inline Vec3f
sampleCosineHemisphere(Vec2f uv, float& pdf)
{
        float theta =
        ...
        // cart = Vec3f(e * std::cos(phi), std::sqrt(1 - uv[0]), e *
        // std::sin(phi));
...
inline Vec3f
sampleSphere(Vec2f uv, float& pdf)
{
