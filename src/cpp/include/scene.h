...
boost::shared_ptr<BxDF> createBxDF(tinyobj::material_t &material) {
    ...
    switch (material.illum) {
        ...
        case 6:
            // leaf
            material.ior = 1.425f; // Source: https://opg.optica.org/ao/abstract.cfm?uri=ao-13-1-109
...
boost::shared_ptr<AreaLight> createAreaLight(
        tinyobj::material_t &material, Triangle *tri) {
...
public:
    ...
    void addFaceInfosMat(std::vector<float> vertices, std::vector<long> indices, std::vector<float> normals, Material mat) {
        for (long &i : indices) {
            i += nVertices();
        }
        ...
        this->normals.insert(std::end(this->normals), std::begin(normals), std::end(normals));

        // populate materials
        for (size_t faceID = nFaces() - (indices.size()/3); faceID < nFaces(); ++faceID) {
    ...
    void addLight(std::vector<float> newVertices, std::vector<long> newIndices, std::vector<float> newNormals, float intensity, Vec3f color) {
        for (long & i : newIndices) {
            i += nVertices();
        }

        this->vertices.insert(std::end(this->vertices), std::begin(newVertices), std::end(newVertices));
        this->indices.insert(std::end(this->indices), std::begin(newIndices), std::end(newIndices));
        this->normals.insert(std::end(this->normals), std::begin(newNormals), std::end(newNormals));
        // populate  triangles
        for (size_t faceID = nFaces() - (newIndices.size()/3); faceID < nFaces(); ++faceID) {
            tinyobj::material_t m;
    ...
    void loadModel(const std::string &filename) {
        ...
        if (!reader.ParseFromFile(filepath.generic_string(), reader_config)) {
            if (!reader.Error().empty()) {
                std::cout<<"[Scene] failed to load "<<filepath.generic_string()<<": "<< reader.Error();
            }
            return;
        }
        ...
        for (const auto & shape : shapes) {
            ...
            for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); ++f) {
                const size_t fv =
                        static_cast<size_t>(shape.mesh.num_face_vertices[f]);
