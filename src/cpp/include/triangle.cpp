...
Vec3f
Triangle::getVertexPosition(uint32_t vertexID) const
{
        return { vertices[3 * vertexID + 0],
...
Vec3f
Triangle::getVertexNormal(uint32_t vertexID) const
{
        return { normals[3 * vertexID + 0],
...
Vec3ui
Triangle::getIndices() const
{
        return { indices[3 * faceID + 0],
                 indices[3 * faceID + 1],
                 indices[3 * faceID + 2] };
}
...
Vec3f
Triangle::getGeometricNormal() const
{
...
Vec3f
Triangle::computeShadingNormal(Vec2f& barycentric) const
{
        Vec3ui vidx = getIndices();
        Vec3f n1 = getVertexNormal(vidx[0]);
        Vec3f n2 = getVertexNormal(vidx[1]);
        Vec3f n3 = getVertexNormal(vidx[2]);
        return n1 * (1.0f - barycentric[0] - barycentric[1]) +
               n2 * barycentric[0] + n3 * barycentric[1];
}

// sample point on the triangle
SurfaceInfo
Triangle::samplePoint(Sampler& sampler, float& pdf) const
{
        SurfaceInfo ret;
