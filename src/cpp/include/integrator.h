...
public:
  ...
  std::vector<PhotonMap> getLightsCausticPhotonMap() const {
    return lightsCausticsPhotonMap;
  }
  ...
  void build(const Scene &scene, Sampler &sampler) override {
    ...
    // allocate a photonmap for each lights in the scene
...
#pragma omp parallel for
      for (unsigned int i = 0; i < nPhotonsGlobal; ++i) {
...
#ifdef __OUTPUT__
              spdlog::info("[PhotonMapping] photon throughput is NaN");
#endif
              break;
          } else if (throughput[0] < 0 || throughput[1] < 0 ||
                     throughput[2] < 0) {
#ifdef __OUTPUT__
              std::cerr<<"[PhotonMapping] photon throughput is
minus"<<std::endl; #endif break;
          }*/
...
#pragma omp critical
      ...
      lightsPhotonMap[l].setPhotons(photonsLights);
        ...
        for (unsigned int i = 0; i < nPhotonsCaustics; ++i) {
          ...
          for (unsigned int k = 0; k < maxDepth; ++k) {
            /*if (std::isnan(throughput[0]) || std::isnan(throughput[1]) ||
                std::isnan(throughput[2])) {
#ifdef __OUTPUT__
                std::cout<<"[PhotonMapping] photon throughput is
NaN"<<std::endl; #endif break; } else if (throughput[0] < 0 || throughput[1] < 0
|| throughput[2] < 0) { #ifdef __OUTPUT__ std::cout<<"[PhotonMapping] photon
throughput is minus"<<std::endl; #endif break;
            }*/
